...
export default class Sketch {
  ...
  mouseEvents() {
    this.container.addEventListener('mousemove', (e) => {
      ...
      console.log(this.mouse.x,this.mouse.y, 99)
  ...
  regenerateGrid() {
    ...
    const data = new Float32Array(3 * size);  // Initialize with 0 by default
  
    for (let i = 0; i < size; i++) {
      const stride = i * 3;
  
      // Set initial values to 0 to avoid any initial distortion effect
      data[stride] = 0;    // No horizontal offset
      data[stride + 1] = 0;  // No vertical offset
      data[stride + 2] = 0;  // No depth offset
    }
  
    // Create DataTexture with the updated data (no random offsets)
